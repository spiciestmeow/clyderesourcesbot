...
async def handle_callback(update: Update):
    ...
    # Step 1: Inventory Choice Menu (Edits current message)
    ...
    # Step 2: Filtered Inventory Result (Deletes and sends new)
    elif query.data.startswith("vamt_filter_"):
        category = query.data.replace("vamt_filter_", "")
        
        # Original logic: Delete old message to keep chat clean
        ...
        await tg_app.bot.edit_message_caption(chat_id=loading_msg.chat_id, message_id=loading_msg.message_id, caption="🍃 <i>The trees whisper... counting hidden treasures...</i>", parse_mode='HTML')
        
        await asyncio.sleep(1.5)
        await tg_app.bot.edit_message_caption(chat_id=loading_msg.chat_id, message_id=loading_msg.message_id, caption=f"✨ <i>Ancient magic is revealing the {category.upper()} inventory...</i>", parse_mode='HTML')
    ...
    # Step 3: Main Menu Navigation (Original style: Delete & Send)
    elif query.data in ["show_main_menu", "main_menu"]:
        try: await query.message.delete()
        except: pass
...
def webhook():
    ...
    async def process_update():
        ...
        # STABLE COMMAND HANDLING (Same as your original)
